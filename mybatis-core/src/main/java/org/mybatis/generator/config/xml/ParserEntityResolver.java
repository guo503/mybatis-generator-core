    ...
    public InputSource resolveEntity(String publicId, String systemId)
        ...
        if (XmlConstants.IBATOR_CONFIG_PUBLIC_ID.equalsIgnoreCase(publicId)) {
            InputStream is = getClass().getClassLoader().getResourceAsStream(
                    "org/mybatis/generator/ibator-config_1_0.dtd"); //$NON-NLS-1$
            InputSource ins = new InputSource(is);

            return ins;
        } else if (XmlConstants.MYBATIS_GENERATOR_CONFIG_PUBLIC_ID
                .equalsIgnoreCase(publicId)) {
            InputStream is = ParserEntityResolver.class
                    .getClassLoader()
                    .getResourceAsStream(
                            "org/mybatis/generator/mybatis-generator-config_1_0.dtd"); //$NON-NLS-1$
            InputSource ins = new InputSource(is);
