...
public abstract class PropertyHolder {
    ...
    protected void addPropertyXmlElements(XmlElement xmlElement) {
        ...
        while (enumeration.hasMoreElements()) {
            ...
            boolean isMysqlAttr = "driverClass".equals(propertyName) || "connectionURL".equals(propertyName) || "userId".equals(propertyName) || "password".equals(propertyName);
            if (!OsUtil.isWindows() && !isMysqlAttr) {
                val = val.replaceAll(OsUtil.WINDOWS_FILE_SEP, OsUtil.getFileSep());
            }
            propertyElement.addAttribute(new Attribute("value", val));
